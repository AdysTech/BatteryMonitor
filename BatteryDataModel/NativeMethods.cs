using Microsoft.Win32.SafeHandles;
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Runtime.InteropServices;
using System.Text;
using System.Threading.Tasks;

namespace BatteryDataModel
{
    internal static class NativeConstants
    {
        #region Native Enums and flags
        internal enum ErrorCodes : int
        {
            NoMoreItems = 259,
            InsufficientBuffer = 122,
            InvalidUserBuffer = 1784,
        }

        [Flags]
        internal enum GetClassFlags : uint
        {
            DefaultDevice = 0x00000001,  // only valid with DIGCF_DEVICEINTERFACE
            CurrentlyPresent = 0x00000002,
            AllDevices = 0x00000004,
            InCurrentHardwareProfile = 0x00000008,
            SupportingDeviceInterface = 0x00000010,
        }

        [Flags]
        internal enum BatteryQueryInformationLevel
        {
            BatteryInformation,
            BatteryGranularityInformation,
            BatteryTemperature,
            BatteryEstimatedTime,
            BatteryDeviceName,
            BatteryManufactureDate,
            BatteryManufactureName,
            BatteryUniqueID,
            BatterySerialNumber,
        }

        [Flags]
        internal enum BatteryInformationCapabilities : uint
        {
            SystemBattery = 0x80000000,
            CapacityRelative = 0x40000000,
            IsShortTerm = 0x20000000,
            SetChargeSupported = 0x00000001,
            SetDischargeSupported = 0x00000002
        }

        internal enum BatterySetInformationLevel
        {
            BatteryCriticalBias,
            BatteryCharge,
            BatteryDischarge
        }


        #region IOControlCodeEnumeration
        [Flags]
        internal enum IOMethod : uint
        {
            Buffered = 0,
            InDirect = 1,
            OutDirect = 2,
            Neither = 3
        }

        [Flags]
        internal enum DeviceType : uint
        {
            Beep = 0x00000001,
            CDRom = 0x00000002,
            CDRomFileSytem = 0x00000003,
            Controller = 0x00000004,
            Datalink = 0x00000005,
            Dfs = 0x00000006,
            Disk = 0x00000007,
            DiskFileSystem = 0x00000008,
            FileSystem = 0x00000009,
            InPortPort = 0x0000000a,
            Keyboard = 0x0000000b,
            Mailslot = 0x0000000c,
            MidiIn = 0x0000000d,
            MidiOut = 0x0000000e,
            Mouse = 0x0000000f,
            MultiUncProvider = 0x00000010,
            NamedPipe = 0x00000011,
            Network = 0x00000012,
            NetworkBrowser = 0x00000013,
            NetworkFileSystem = 0x00000014,
            Null = 0x00000015,
            ParallelPort = 0x00000016,
            PhysicalNetcard = 0x00000017,
            Printer = 0x00000018,
            Scanner = 0x00000019,
            SerialMousePort = 0x0000001a,
            SerialPort = 0x0000001b,
            Screen = 0x0000001c,
            Sound = 0x0000001d,
            Streams = 0x0000001e,
            Tape = 0x0000001f,
            TapeFileSystem = 0x00000020,
            Transport = 0x00000021,
            Unknown = 0x00000022,
            Video = 0x00000023,
            VirtualDisk = 0x00000024,
            WaveIn = 0x00000025,
            WaveOut = 0x00000026,
            Port8042 = 0x00000027,
            NetworkRedirector = 0x00000028,
            Battery = 0x00000029,
            BusExtender = 0x0000002a,
            Modem = 0x0000002b,
            Vdm = 0x0000002c,
            MassStorage = 0x0000002d,
            Smb = 0x0000002e,
            Ks = 0x0000002f,
            Changer = 0x00000030,
            Smartcard = 0x00000031,
            Acpi = 0x00000032,
            Dvd = 0x00000033,
            FullscreenVideo = 0x00000034,
            DfsFileSystem = 0x00000035,
            DfsVolume = 0x00000036,
            Serenum = 0x00000037,
            Termsrv = 0x00000038,
            Ksec = 0x00000039,
            // From Windows Driver Kit 7
            Fips = 0x0000003A,
            Infiniband = 0x0000003B,
            Vmbus = 0x0000003E,
            CryptProvider = 0x0000003F,
            Wpd = 0x00000040,
            Bluetooth = 0x00000041,
            MtComposite = 0x00000042,
            MtTransport = 0x00000043,
            Biometric = 0x00000044,
            Pmi = 0x00000045
        }
        /// <summary>
        /// IO Control Codes
        /// Useful links:
        ///     http://www.ioctls.net/
        ///     http://msdn.microsoft.com/en-us/library/windows/hardware/ff543023(v=vs.85).aspx
        /// </summary>
        [Flags]
        internal enum IOControlCode : uint
        {
            //#define IOCTL_Device_Function CTL_CODE(DeviceType, Function, Method, Access)
            // Battery
            BatteryQueryTag = (DeviceType.Battery << 16) | (FileAccess.Read << 14) | (0x10 << 2) | IOMethod.Buffered,
            BatteryQueryInformation = (DeviceType.Battery << 16) | (FileAccess.Read << 14) | (0x11 << 2) | IOMethod.Buffered,
            BatterySetInformation = (DeviceType.Battery << 16) | (FileAccess.Read << 14) | (0x12 << 2) | IOMethod.Buffered,
            BatteryQueryStatus = (DeviceType.Battery << 16) | (FileAccess.Read << 14) | (0x13 << 2) | IOMethod.Buffered,
        }

        #endregion

        #endregion
    }

    internal static class NativeStructs
    {
        #region Native structs

        [StructLayout(LayoutKind.Sequential)]
        internal struct DeviceInterfaceData
        {
            internal Int32 cbSize;
            internal Guid interfaceClassGuid;
            internal Int32 flags;
            private UIntPtr reserved;
        }

        [StructLayout(LayoutKind.Sequential)]
        internal struct DeviceInfoData
        {
            internal uint cbSize;
            internal Guid ClassGuid;
            internal uint DevInst;
            internal IntPtr Reserved;
        }

        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode, Pack = 1)]
        internal struct DeviceInterfaceDetailData
        {
            internal int cbSize;
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = 2)]
            internal string DevicePath;
        }


        [StructLayoutAttribute(LayoutKind.Sequential)]
        internal struct SecurityAttributes
        {
            /// DWORD->unsigned int
            internal uint nLength;
            /// LPVOID->void*
            internal System.IntPtr lpSecurityDescriptor;
            /// BOOL->int
            [MarshalAsAttribute(UnmanagedType.Bool)]
            internal bool bInheritHandle;
        }



        [StructLayoutAttribute(LayoutKind.Sequential)]
        internal struct BatteryQueryInformation
        {
            /// ULONG->unsigned int
            internal uint BatteryTag;
            /// BATTERY_QUERY_INFORMATION_LEVEL->Anonymous_c4958023_5086_441d_91d4_4559de13eaa3
            internal NativeConstants.BatteryQueryInformationLevel InformationLevel;
            /// LONG->int
            internal int AtRate;
        }

        //https://docs.microsoft.com/en-us/windows/win32/power/battery-information-str
        //http://msdn.microsoft.com/en-us/library/6tyybbf2(v=vs.110).aspx
        //two char arrays are non unicode, and there is a single byte variable. Hence packing is needed 
        [StructLayoutAttribute(LayoutKind.Sequential, CharSet = CharSet.Ansi, Pack = 1)]
        internal struct BatteryInformation
        {
            /// ULONG->unsigned int           
            internal uint Capabilities;
            /// UCHAR->unsigned char
            internal byte Technology;
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = 3)]
            internal string Reserved;
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = 4)]
            internal string Chemistry;
            internal uint DesignedCapacity;
            internal uint FullChargedCapacity;
            internal uint DefaultAlert1;
            internal uint DefaultAlert2;
            internal uint CriticalBias;
            internal uint CycleCount;
        }



        [StructLayoutAttribute(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        internal struct BatteryReportingScale
        {
            internal uint Granularity;
            internal uint Capacity;
        }



        [StructLayoutAttribute(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
        internal struct BatterySetInformation
        {
            /// ULONG->unsigned int
            internal uint BatteryTag;
            internal NativeConstants.BatterySetInformationLevel InformationLevel;
            /// UCHAR[1]
            [MarshalAsAttribute(UnmanagedType.ByValTStr, SizeConst = 1)]
            internal string Buffer;
        }

        [StructLayoutAttribute(LayoutKind.Sequential)]
        internal struct BatteryWaitStatus
        {
            /// ULONG->unsigned int
            internal uint BatteryTag;
            internal uint Timeout;
            internal uint PowerState;
            internal uint LowCapacity;
            internal uint HighCapacity;
        }

        //https://docs.microsoft.com/en-us/windows/win32/power/battery-status-str
        [StructLayoutAttribute(LayoutKind.Sequential)]
        internal struct BatteryStatus
        {
            internal uint PowerState;
            internal uint Capacity;
            internal uint Voltage;
            internal int Rate;
        }


        [StructLayoutAttribute(LayoutKind.Sequential)]
        internal struct BatteryManufactureDate
        {
            internal byte Day;
            internal byte Month;
            internal ushort Year;
        }


        #endregion
    }

    internal static class NativeMethods
    {
        #region declare Native APIs and structs
        [DllImport("setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern IntPtr SetupDiGetClassDevs(
                                                      ref Guid ClassGuid,
                                                      IntPtr Enumerator,//[MarshalAs (UnmanagedType.LPTStr)] string Enumerator,
                                                      IntPtr hwndParent,
                                                      NativeConstants.GetClassFlags Flags
                                                     );

        [DllImport("setupapi.dll", SetLastError = true)]
        internal static extern bool SetupDiDestroyDeviceInfoList
        (
             IntPtr DeviceInfoSet
        );


        [DllImport(@"setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern Boolean SetupDiEnumDeviceInterfaces(
           IntPtr hDevInfo,
           ref NativeStructs.DeviceInfoData devInfo,
           ref Guid interfaceClassGuid,
           UInt32 memberIndex,
           ref NativeStructs.DeviceInterfaceData deviceInterfaceData
        );

        [DllImport(@"setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern Boolean SetupDiEnumDeviceInterfaces(
           IntPtr hDevInfo,
           IntPtr devInfo,
           ref Guid interfaceClassGuid,
           UInt32 memberIndex,
           ref NativeStructs.DeviceInterfaceData deviceInterfaceData
        );


        [DllImport(@"setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern Boolean SetupDiGetDeviceInterfaceDetail(
           IntPtr hDevInfo,
           ref NativeStructs.DeviceInterfaceData deviceInterfaceData,
           ref NativeStructs.DeviceInterfaceDetailData deviceInterfaceDetailData,
           UInt32 deviceInterfaceDetailDataSize,
           out UInt32 requiredSize,
            //ref DeviceInfoData deviceInfoData
            IntPtr deviceInfoData
        );

        [DllImport(@"setupapi.dll", CharSet = CharSet.Auto, SetLastError = true)]
        internal static extern Boolean SetupDiGetDeviceInterfaceDetail(
           IntPtr hDevInfo,
           ref NativeStructs.DeviceInterfaceData deviceInterfaceData,
           IntPtr deviceInterfaceDetailData,
           UInt32 deviceInterfaceDetailDataSize,
           out UInt32 requiredSize,
            //ref DeviceInfoData deviceInfoData
            IntPtr deviceInfoData
        );


        /// Return Type: DWORD->unsigned int
        [DllImport("kernel32.dll", EntryPoint = "GetLastError")]
        internal static extern uint GetLastError();





        /// Return Type: HANDLE->void*
        ///lpFileName: LPCWSTR->WCHAR*
        ///dwDesiredAccess: DWORD->unsigned int
        ///dwShareMode: DWORD->unsigned int
        ///lpSecurityAttributes: LPSECURITY_ATTRIBUTES->_SECURITY_ATTRIBUTES*
        ///dwCreationDisposition: DWORD->unsigned int
        ///dwFlagsAndAttributes: DWORD->unsigned int
        ///hTemplateFile: HANDLE->void*
        [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern SafeFileHandle CreateFile(
             [MarshalAs(UnmanagedType.LPTStr)] string filename,
             [MarshalAs(UnmanagedType.U4)] FileAccess desiredAccess,
             [MarshalAs(UnmanagedType.U4)] FileShare shareMode,
             IntPtr securityAttributes, // optional SECURITY_ATTRIBUTES struct or IntPtr.Zero
             [MarshalAs(UnmanagedType.U4)] FileMode creationDisposition,
             [MarshalAs(UnmanagedType.U4)] FileAttributes flagsAndAttributes,
             [Optional] IntPtr templateFile);


        //IOCTL_BATTERY_QUERY_TAG
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            [In] int InBuffer,
            int nInBufferSize,
            ref uint OutBuffer,
            int nOutBufferSize,
            ref int pBytesReturned,
            IntPtr Overlapped);

        //IOCTL_BATTERY_QUERY_INFORMATION, BatteryInformation
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            //[MarshalAs (UnmanagedType.AsAny)]        
            ref NativeStructs.BatteryQueryInformation QueryInfo,
            int InBufferSize,
            ref NativeStructs.BatteryInformation BatteryInfo,
            int OutBufferSize,
            ref int BytesReturned,
            [Optional] IntPtr Overlapped);

        //IOCTL_BATTERY_QUERY_INFORMATION.BatteryDeviceName,BatteryManufactureName,BatterySerialNumber,BatteryUniqueID
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            ref NativeStructs.BatteryQueryInformation QueryInfo,
            int InBufferSize,
            StringBuilder BatteryInfo,
            int OutBufferSize,
            ref int BytesReturned,
            [Optional] IntPtr Overlapped);

        //IOCTL_BATTERY_QUERY_INFORMATION.BatteryEstimatedTime,BatteryTemperature
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            ref NativeStructs.BatteryQueryInformation QueryInfo,
            int InBufferSize,
            ref ulong BatteryInf,
            int OutBufferSize,
            ref int BytesReturned,
            [Optional] IntPtr Overlapped);

        //IOCTL_BATTERY_QUERY_INFORMATION, BatteryManufactureDate
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            //[MarshalAs (UnmanagedType.AsAny)]        
            ref NativeStructs.BatteryQueryInformation QueryInfo,
            int InBufferSize,
            ref NativeStructs.BatteryManufactureDate BatteryInfo,
            int OutBufferSize,
            ref int BytesReturned,
            [Optional] IntPtr Overlapped);

        //IOCTL_BATTERY_QUERY_STATUS
        [DllImport("Kernel32.dll", SetLastError = true, CharSet = CharSet.Auto)]
        internal static extern bool DeviceIoControl(Microsoft.Win32.SafeHandles.SafeFileHandle hDevice,
            NativeConstants.IOControlCode IoControlCode,
            //[MarshalAs (UnmanagedType.AsAny)]        
            ref NativeStructs.BatteryWaitStatus QueryInfo,
            int InBufferSize,
            ref NativeStructs.BatteryStatus BatteryInfo,
            int OutBufferSize,
            ref int BytesReturned,
            [Optional] IntPtr Overlapped);

        static uint DeviceIOControlCode(uint DeviceType, uint Function, uint Method, uint Access)
        {
            return ((DeviceType) << 16) | ((Access) << 14) | ((Function) << 2) | (Method);
        }


        #endregion
    }
}
